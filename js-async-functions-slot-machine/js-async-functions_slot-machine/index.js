...
spinButton.addEventListener("click", async () => {
  try {
    ...
    // get number of identical symbols
    const maxCount = getMaxCount(values);
    let totalPoints = 0;

    // set score
    if (maxCount === 3) totalPoints += 100;
    else if (maxCount === 2) totalPoints += 10;
    else totalPoints = 0;
    result.setResult(totalPoints);
